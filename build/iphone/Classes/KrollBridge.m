...
NSString * Tierstimmenarchiv$ModuleRequireFormat = @"(function(exports){"
...
@implementation TierstimmenarchivObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_tierstimmenarchiv);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Tierstimmenarchiv global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Tierstimmenarchiv" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_tierstimmenarchiv = [[TierstimmenarchivObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_tierstimmenarchiv];
	
	NSString *_tierstimmenarchivNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _tierstimmenarchivNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_tierstimmenarchiv valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Tierstimmenarchiv$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.tierstimmenarchiv.kroll" 
